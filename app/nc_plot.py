...
def create_ts_plot(data):
    ...
    p.line(x=data.index.name,
           y=data.columns[0],
           source=source,
           line_alpha=0.6,
           color='RoyalBlue',
           )
    p.circle(x=data.index.name,
             y=data.columns[0],
             source=source,
             color='RoyalBlue',
             size=3,
...
def create_page(data, metadata=True):
    ...
    if metadata:
        ...
        """ """
        #title_div = Div(text="""<h1><b>{title}</b></h1>""".format(title=title), width_policy='fit', css_classes=["bk"], style={"position": 'absolute', 'left': '254px', 'top': '5px', 'width': '100%', 'height': '984px', 'display': 'block'})
        #dataset_metadata_div = Div(text="""{dataset_metadata}""".format(
        #    dataset_metadata=dataset_metadata), width_policy='fit')
        #variable_metadata_div = Div(
        #    text="""<p>{variable_metadata}</p>""".format(variable_metadata=variable_metadata), width_policy='fit')
        """ """
        title_content = """<h1><b>{title}</b></h1><br>""".format(title=title)
        dt_metadata_content = """{dataset_metadata}""".format(dataset_metadata=dataset_metadata)
        vr_metadata_content = """<p>{variable_metadata}</p>""".format(variable_metadata=variable_metadata)
        md_div_content = title_content+dt_metadata_content+vr_metadata_content
        md_div = Div(text=md_div_content, css_classes=["bk"], style={'width': '100%', 'display': 'block'})
